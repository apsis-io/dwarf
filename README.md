...
`wit.Checkpoint.snapshot()` dumps this component's entire WASM linear
memory — the QuickJS heap, engine bookkeeping, every JS global — as a single
`Uint8Array`, with no hand-written serialization:

```js
const bytes = wit.Checkpoint.snapshot(); // Uint8Array — the full memory image
```

This is the same principle [Wizer](https://github.com/bytecodealliance/wizer)
already uses to give dwarf-built components a fast cold start (a build-time
memory snapshot baked into the shipped component), just triggered at runtime
instead. It's a pure read — safe to call at any time, doesn't disturb the
running component — useful for diagnostics or offline state inspection.

**There is deliberately no `restore()`.** It was attempted and proven
fundamentally unsound, not just difficult, through two independent failure
modes:

- Restoring a memory image into a call that's still executing overwrites that
  call's own return path (return addresses, saved locals live in the same
  linear memory being replaced) — confirmed empirically with a test that
  restored into the *same still-running instance* (not a different one, ruling
  out any cross-instance state mismatch) and reliably crashed.
- Working around that by trapping instead of returning (so control leaves via
  wasmtime's host-side unwind mechanism, not guest stack data) runs straight
  into the Component Model's own reentrance guard: wasmtime permanently
  refuses further calls into a component instance after any call traps
  (`may_enter` is cleared on entry and only reset by a normal completion —
  confirmed at the source level in
  `wasmtime-46.0.1/src/runtime/component/func.rs:445`). This is canonical-ABI
  spec behavior every compliant host enforces, not a wasmtime gap to route
  around.

Both are fundamental to how the component model defines call safety. The only
architecturally sound way to restore a memory image is to inject it *before
any code runs* — the same guarantee Wizer's own build-time snapshot relies on
— which requires raw core-module memory access that the component model
deliberately hides from a host embedding a `Component` (the same
encapsulation "`component::Instance` has no `get_memory`; only core `Instance`
does" that Periapsis's own ADR-0028 documents as the reason they built
app-level cooperative checkpoint/restore instead of a transparent one).

**For actual checkpoint/restore, use application-level serialization** — e.g.
against a host's own cooperative interface, such as Periapsis's Trail runtime,
which imports `periapsis:component/checkpoint` (`requested() -> bool`,
`save(list<u8>)`, `load() -> option<list<u8>>`) and calls `save` right before
a coordinated restart, handing the bytes back via `load` on the relaunched
instance. `wit.Checkpoint.snapshot()` doesn't help construct those bytes
generically for the reasons above; serialize the specific state you care
about by hand (`JSON.stringify` a few globals is usually enough).

**Host-side sizing note.** A snapshot is the whole linear memory image — easily
tens of MB even for a trivial component (QuickJS's own heap baseline).
wasmtime's default 128 MiB "hostcall fuel" budget (a DoS guard on guest-to-host
data volume, which covers data returned from an export call, not just
arguments passed into a host import) is sized for ordinary WIT payloads, not
repeated whole-memory dumps. A host that wants to call `snapshot()` more than
once or twice per instance needs to raise it via
[`Store::set_hostcall_fuel`](https://docs.rs/wasmtime/latest/wasmtime/component/struct.Store.html#method.set_hostcall_fuel).

See `examples/checkpoint.{wit,js}` for a self-contained, Periapsis-independent
demonstration of `snapshot()` you can run by hand with `wasmtime run --invoke`.
